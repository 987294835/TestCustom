				...
				<div class="tcs-top-nav-container">
					<div class="row line-height-50">
						<nav class="tcs-top-nav col-lg-12 col-md-12">
							<ul>
								<li>
									<a href="Select_exam.do">我的试卷</a>
								</li>
								<li>
									<a class="active margin-right-25" href="">添加的岗位</a>
								</li>
								<li>
									<a href="Select_Collect.do">我的收藏</a>
								</li>
							</ul>
		...
		<div class="profile-person-overlay"></div>
									...
									<c:forEach var="c" items="${jobs}">
										<tr>
											...
											<td class="text-left" style="width: 25%;">
												<a href="see_persons.do?job_id=${c.job_id }">${c.job_name}</a>
											</td>
											<td style="width: 16%;">${c.job_time }</td>
											<td><a href="see_job.do?job_id=${c.job_id }" class="tcs-link">编辑</a></td>
											<td><a href="delete_job.do?job_id=${c.job_id }" class="tcs-link">删除</a></td>
											<td>
												<c:if test="${c.job_state == 0}"><a href="publish_job.do?job_id=${c.job_id }&i=0" class="tcs-link">发布岗位</a></c:if>
										  		<c:if test="${c.job_state == 1}"><a href="publish_job.do?job_id=${c.job_id }&i=1" class="tcs-link">取消发布</a></c:if>
											</td>
											<td>
												 <c:if test="${c.exam_id > 0 }">
										 			<a href="query_exam_titles.do?exam_id=${c.exam_id }">试卷ID:${c.exam_id}</a>
												 </c:if>
												 <c:if test="${c.exam_id == 0 }">未绑定试卷</c:if>
											</td>
											<td><a href="send_exam_code.do?job_id=${c.job_id }" class="tcs-link">通知考试</a></td>
										</tr>
										</c:forEach>
									</tbody>
								</table>
			...
			<div id="PaginationBar_info" class="page">
				...
				<c:choose>
					...
					<c:otherwise>
						<a href="job_page_change.do?currPage=${pc.firstPage }">首页</a>
					</c:otherwise>
				</c:choose>
				...
				<c:choose>
					<c:when test="${pc.hasPrevious }">
						<a href="job_page_change.do?currPage=${pc.currentPage-1 }">上一页</a>
					</c:when>
					<c:otherwise>
						<span>上一页</span>
					</c:otherwise>
				...
				<c:choose>
					<c:when test="${pc.hasNext }">
						<a href="job_page_change.do?currPage=${pc.currentPage+1 }">下一页</a>
					</c:when>
					<c:otherwise>
						<span>下一页</span>
					</c:otherwise>
				...
				<c:choose>
					...
					<c:otherwise>
						<a href="job_page_change.do?currPage=${pc.totalPages }">尾页</a>
					</c:otherwise>
				</c:choose>
			</div>
		</div>
